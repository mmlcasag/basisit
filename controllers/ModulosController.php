...
class ModulosController extends BaseController {
    ...
    private function exibirTelaListar($dados = array()) {
        $view = new View('views/listarModulos.phtml');
    ...
    private function carregarDadosManter($connection, $id = "", $mensagem = "") {
        if (!Functions::isEmpty($id)) {
            $model = new ModulosModel();
            $modulos = $model->loadById($connection, $id);
        } else {
            $modulos = new ModulosVo();
        }
        return $this->trabalharDadosManter($modulos, $mensagem);
    }
    
    private function trabalharDadosManter($modulos, $mensagem = "") {
        return array( 'mensagem' => $mensagem
                    , 'registro' => $modulos );
    }
    
    private function exibirTelaManter($dados = array()) {
        $view = new View('views/manterModulos.phtml');
        $view->setParams($dados);
        $view->showContents();
    }
